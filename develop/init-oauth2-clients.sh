...
docker run -it --rm --network host -e HYDRA_ADMIN_URL=http://localhost:9001 ${HYDRA_IMAGE} \
  clients create \
    ...
    --response-types code \
    --scope openid,offline,odp.catalogue:read,odp.client:admin,odp.client:read,odp.collection:admin,odp.collection:read,odp.project:admin,odp.project:read,odp.provider:admin,odp.provider:read,odp.record:admin,odp.record:create,odp.record:manage,odp.record:read,odp.role:admin,odp.role:read,odp.schema:read,odp.scope:read,odp.tag:read,odp.user:admin,odp.user:read \
