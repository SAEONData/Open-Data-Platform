...
# This script (re)creates OAuth2 clients for the ODP UIs.
# It should be run from ../deploy or ../develop, as applicable.
...
docker run -it --rm --network host -e HYDRA_ADMIN_URL=${HYDRA_ADMIN_URL} ${HYDRA_IMAGE} \
  clients delete --skip-tls-verify ${ODP_UI_DAP_CLIENT_ID}
...
docker run -it --rm --network host -e HYDRA_ADMIN_URL=${HYDRA_ADMIN_URL} ${HYDRA_IMAGE} \
  clients create --skip-tls-verify \
    ...
    --callbacks ${ODP_UI_DAP_URL}/oauth2/logged_in \
    --post-logout-callbacks ${ODP_UI_DAP_URL}/oauth2/logged_out
